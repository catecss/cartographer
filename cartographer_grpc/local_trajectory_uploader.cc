...
LocalTrajectoryUploader::~LocalTrajectoryUploader() {
  if (imu_client_) {
    CHECK(imu_client_->WritesDone());
    CHECK(imu_client_->Finish().ok());
  }
  if (odometry_client_) {
    CHECK(odometry_client_->WritesDone());
    CHECK(odometry_client_->Finish().ok());
  }
  if (fixed_frame_pose_client_) {
    CHECK(fixed_frame_pose_client_->WritesDone());
    CHECK(fixed_frame_pose_client_->Finish().ok());
  }
  if (local_slam_result_client_) {
    CHECK(local_slam_result_client_->WritesDone());
    CHECK(local_slam_result_client_->Finish().ok());
  }
}
...
void LocalTrajectoryUploader::ProcessLocalSlamResultDataMessage(
  ...
  CHECK(local_slam_result_client_->Write(*data_request));
}
