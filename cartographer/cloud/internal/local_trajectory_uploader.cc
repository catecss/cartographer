...
class LocalTrajectoryUploader : public LocalTrajectoryUploaderInterface {
 public:
  LocalTrajectoryUploader(const std::string &uplink_server_address, bool use_ssl, std::unique_ptr<framework::auth::CredentialsProvider> credentials_provider)
      : client_channel_(std::make_shared<framework::Channel>(uplink_server_address, use_ssl, std::move(credentials_provider))) {}
...
std::unique_ptr<LocalTrajectoryUploaderInterface> CreateLocalTrajectoryUploader(
  ...
  return make_unique<LocalTrajectoryUploader>(uplink_server_address, use_ssl, std::move(credentials_provider));
}
